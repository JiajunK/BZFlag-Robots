...
AStarGraph::AStarGraph(const float startPos[3], const float goalPos[3])
{
  if (closedArraySize > closedArrayFixedSize) {
	...
	/** error resolved(finalProj): replaced sprintf_s() with C++ standard equivalent snprintf()
	 sprintf_s(buffer, "***Astar.cpp: closedArrayFixedSize is too small, needs to be %i***", closedArraySize);
	 */
    _snprintf_s(buffer, sizeof(buffer), "***Astar.cpp: closedArrayFixedSize is too small, needs to be %i***",
      closedArraySize);
...
void		AStarGraph::aStarSearch(const float startPos[3], const float goalPos[3], vector< AStarNode > & path)
{
	...
	if (path.empty()) {
		 ...
		 */
		_snprintf_s(buffer, sizeof(buffer), "***AStarGraph::aStarSearch: could not find a path from (%f, %f) to (%f, %f)***",
			startPos[0], startPos[1], goalPos[0], goalPos[1]);
		controlPanel->addMessage(buffer);
	}
#ifdef TRACE_ASTAR
...
void		AStarGraph::startAStar(vector< AStarNode > & path)
{
	...
	if (openQueue.empty()) {
		 ...
		 */
		_snprintf_s(buffer, sizeof(buffer), "***AStarGraph::startAStar: empty openQueue (%i, %i) to (%i, %i)***",
			startNode->getX(), startNode->getY(), goalNode->getX(), goalNode->getY());
		controlPanel->addMessage(buffer);
	}
	else {
